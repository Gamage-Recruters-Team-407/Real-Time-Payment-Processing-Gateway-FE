...
export default function RefundRequest() {
  ...
  useEffect(() => {
    if (user && !hasPrefilled.current) {
      const extractedName = user.name || user.fullName || user.fullname || user.displayName || 
  ...
  // Submit Form — uploads image to Cloudinary via backend
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ CORRECTED ENDPOINT: removed '/create' – backend expects POST /refunds
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div className="space-y-1.5 sm:space-y-2">
                      ...
                      <input
                        ...
                        required
                        className="w-full rounded-lg border border-slate-200 bg-slate-100 cursor-not-allowed text-slate-500 px-3.5 sm:px-4 py-2 sm:py-2.5 text-sm placeholder:text-slate-400 focus:outline-none transition-all"
                      />
                    </div>
